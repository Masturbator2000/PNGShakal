...
def FileChooser(ChooserTitle, DefaultDirectory = UserPath + '\\Pictures\\', Filter = 'PNGShakal (*.png *.jpg *.bmp)'):
    log('Opening windows FileChooser Window', 'FilePick')
    UserChoice = QtWidgets.QFileDialog.getOpenFileName(None, ChooserTitle, DefaultDirectory, Filter)
    log('Selected PNGShakal file: "%s"' % UserChoice[0], 'FilePick')
    return UserChoice
log('Function "FileChooser" defined')

def FileSave(ChooserTitle, DefaultDirectory = UserPath + '\\Pictures\\Shakal.png', Filter = 'PNGShakal (*.png *.jpg *.bmp *.psd)'):
    log('Opening windows FileSave Window', 'FileSave')
    UserChoice = QtWidgets.QFileDialog.getSaveFileName(None, ChooserTitle, DefaultDirectory, Filter)
...
def pixelate(image, pixel_size=9, draw_margin=True):
    log('Started function "pixelate" with args: image="%s" pixel_size="%s" draw_margin="%s"' % (image, str(pixel_size), str(draw_margin) ), 'Create')
    margin_color = (0, 0, 0)
    log('margin_color="%s"' % str(margin_color), 'Create')

    image = image.resize((image.size[0] // pixel_size, image.size[1] // pixel_size), Image.NEAREST)
    log('Resized image: image.size[0] // pixel_size, image.size[1] // pixel_size; Image.NEAREST', 'Create')
    image = image.resize((image.size[0] * pixel_size, image.size[1] * pixel_size), Image.NEAREST)
    log('Resized image: image.size[0] * pixel_size, image.size[1] * pixel_size; Image.NEAREST', 'Create')
    pixel = image.load()
    log('Image loaded')
        ...
        for i in range(0, image.size[0], pixel_size):
            for j in range(0, image.size[1], pixel_size):
                for r in range(pixel_size):
                    ...
                    log('Drawing margin, pixel[i+r, j] = %s' % str(pixel[i+r, j]), 'Create')
                    pixel[i, j+r] = margin_color
                    log('Drawing margin, pixel[i, j+r] = %s' % str(pixel[i, j+r]), 'Create')

    return image

def SHAKAL_(File):
    log('Started function "SHAKAL_" with args: File="%s"' % File, 'Create')
    log('Temp image file: %s' % UserPath + '\\AppData\\Local\\Temp\\ShakalImage-Temp-PNG.png', 'Create')
    Ui_Form.File = File
    ...
    PNG.save(UserPath + '\\AppData\\Local\\Temp\\ShakalImage-Temp-PNG.png', 'PNG')
    ...
    NewPNGSize = ( int(PNGSize[0] / PNGQualityBad),  int(PNGSize[1] / PNGQualityBad) )
    ...
    if DropTF(): log('Applying effect ImageFilter.MinFilter 5', 'Create'); PNG = PNG.filter( ImageFilter.MinFilter(5) )
    if DropTF(): log('Applying effect ImageFilter.DETAIL', 'Create'); PNG = PNG.filter( ImageFilter.DETAIL )
    if DropTF(): log('Applying effect ImageFilter.EDGE_ENHANCE_MORE', 'Create'); PNG = PNG.filter( ImageFilter.EDGE_ENHANCE_MORE )
    if DropTF(): log('Applying effect ImageFilter.RankFilter 3 0', 'Create'); PNG = PNG.filter( ImageFilter.RankFilter(3, 0) )
    if DropTF(): log('Applying effect ImageFilter.BLUE', 'Create'); PNG = PNG.filter( ImageFilter.BLUR )
    ...
    if DropTF():
        ...
        im = Image.open(UserPath + '\\AppData\\Local\\Temp\\ShakalImage-Temp-PNG.png')
        a = np.asarray(im)
        b = np.zeros_like(a)
        xs = a.shape[1] // 3
        for x in range(xs//2):
...
class Ui_Form(QtWidgets.QMainWindow):
    ...
    def setupUi(self, Form):
        ...
        self.BackgroundFloodFill.setStyleSheet("background-color: rgb(0, 0, 0);")
        ...
        self.RetImage.setAlignment(QtCore.Qt.AlignLeading|QtCore.Qt.AlignLeft|QtCore.Qt.AlignTop)
...
"font: 87 9pt \"Segoe UI Black\";")
    ...
    def save(self):
        ...
        saveto = FileSave('PNGShakal - Сохранение изображения', Filter = 'PNG Картинка (*.png)')[0]
...
class Ui_ApplicationWindow(QtWidgets.QMainWindow):
    def setupUi(self, ApplicationWindow):
        ...
        self.BackgroundFloodFill.setCursor(QtGui.QCursor(QtCore.Qt.ForbiddenCursor))
        self.BackgroundFloodFill.setStyleSheet("background-color: rgb(0, 0, 0);")
        self.BackgroundFloodFill.setText("")
        self.BackgroundFloodFill.setObjectName("BackgroundFloodFill")
...
"padding-bottom: 5;")
        ...
        self.SelectFileButton.setStyleSheet("border-style: solid;\n"
"border-radius: 7;\n"
"border-color: rgb(255, 255, 255);\n"
"background-color: rgb(255, 255, 255);\n"
"border-width: 1;\n"
...
"font: 87 9pt \"Segoe UI Black\";")
    ...
    def retranslateUi(self, ApplicationWindow):
        _translate = QtCore.QCoreApplication.translate
        ApplicationWindow.setWindowTitle(_translate("ApplicationWindow", "PNGShakal 2.0"))
        self.ApplicationName.setText(_translate("ApplicationWindow", "PNGShakal"))
        self.HintFile.setText(_translate("ApplicationWindow", "Расположение картинки"))
        self.SelectFileButton.setText(_translate("ApplicationWindow", "Выбрать файл"))
